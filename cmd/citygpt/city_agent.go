...
func (c *cityAgent) init(cp genai.Provider, cityData fs.FS) error {
	c.c = cp
	c.cityData = cityData
...
func (c *cityAgent) query(ctx context.Context, msgs genai.Messages) (genai.Messages, []string) {
	...
	newMsgs, usage, err := adapters.GenSyncWithToolCallLoop(ctx, c.c, msgs, &optsText, &optsTools, genai.GenOptionsSeed(1))
	...
	if err != nil {
		...
		m := genai.Messages{genai.Message{Replies: []genai.Reply{{Text: "Sorry, there was an error processing your request."}}}}
		return m, files
	}
